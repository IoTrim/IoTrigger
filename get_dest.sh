...
do
    ...
    if [[ "$TAG_DIR" != "notag" ]] && [[ $phone_exp != "x" ]]; then
	 /opt/moniotr/bin/tag-experiment cancel $name "power"
        ...
        echo $LAST_CAPT
        #tshark -r $LAST_CAPT  -q -z hosts | awk 'NF && $1!~/^#/' | awk '{print $2}' | grep -v "ntp" | grep -v "time" | grep -v "dns" | sort | uniq > $TAG_DIR/$name/$name_exp/list_dest_pow
    ...
    COMP2=$(convert $CAPT_DIR/${name}.${name_exp}2.png $CAPT_DIR/${name}.${name_exp}.${DATE}.png -crop $crop +repage miff:- | compare -verbose -metric MAE  - $CAPT_DIR/out.${name}.${name_exp}.${DATE}.png 2>&1 | grep all | awk '{print $2}')
